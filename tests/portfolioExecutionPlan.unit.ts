...
ok(plan.lines.some(x => x.action === 'BUY_ETF' && x.shares === 4 && x.amountEur === 160), 'ETF contribution becomes whole-share buy instruction');
...
ok(plan.lines.some(x => x.action === 'SELL_ETF' && x.shares === 2 && x.amountEur === 100), 'ETF overweight becomes quantified partial sale');
...
console.log(`Portfolio Execution Plan: ${passed}/${passed} execution/transfer/whole-share invariants passed.`);
