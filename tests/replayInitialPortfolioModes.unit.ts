...
// Asset selection must keep the existing catalogue dropdown and also support catalogue/open-market lookup.
requireText(controls, '<select value={assetToAdd}', 'EXISTING_ASSET_DROPDOWN_MUST_REMAIN_AVAILABLE');
requireText(controls, 'EUR_PORTFOLIO_DISCOVERY_UNIVERSE.map(asset => <option', 'EXISTING_ASSET_DROPDOWN_MUST_USE_CANONICAL_UNIVERSE');
requireText(controls, 'Buscar nombre, ticker o ISIN', 'FREE_ASSET_SEARCH_UI_MISSING');
...
requireText(controls, '/api/alerts/asset-discovery/search', 'OPEN_MARKET_SEARCH_MUST_REMAIN_IN_EXISTING_CONTROLS');
