...
import { BarChart3, ChevronDown, Radar, RefreshCw, Search } from 'lucide-react';
import {
...
export const InvestmentResearchLab: React.FC<Props> = ({ scan, decision, requestedSymbol }) => {
  ...
  const loadExternalScan = async () => {
    ...
    try {
      setExternalScan(await AssetUniverseScanner.scan(EUR_VALIDATION_HOLDOUT_UNIVERSE, fiveYearsAgo(), isoDate(new Date()), { forceRefresh: false, concurrency: 3, maxSelected: 10, minimumBars: 252, maxDataAgeDays: 7 }));
    } catch (e: any) {
  ...
  return <div className="space-y-5">
    <section className="rounded-2xl border border-violet-500/25 bg-gradient-to-br from-violet-950/35 via-slate-900 to-slate-950 p-5">
      ...
      <div className="mt-4 grid gap-2 md:grid-cols-[1fr_auto]">
        ...
        <div className="rounded-xl border border-slate-700 bg-slate-950 p-3 text-[10px] text-slate-400">También puedes escribir directamente <b className="text-white">AAPL, NVDA, SAN.MC, SAP.DE…</b> o un ISIN en el analizador inferior. No se lanza ningún replay pesado hasta que tú eliges qué analizar.</div>
      </div>
    </section>

    <section className="rounded-2xl border border-slate-800 bg-slate-900 p-5">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div><div className="flex items-center gap-2"><BarChart3 className="h-5 w-5 text-indigo-300"/><h2 className="font-bold text-white">Radar actual ampliado</h2></div><p className="mt-1 text-xs text-slate-400">El radar de producción está disponible inmediatamente. El universo externo de robustez se carga solo si lo pides, para que no bloquee la escritura, las gráficas ni la navegación normal.</p></div><div className="flex flex-wrap gap-2">{(['OPPORTUNITY','MOMENTUM','SAFETY','PUNISHED'] as RankingMode[]).map(mode => <button key={mode} onClick={() => setRankingMode(mode)} className={`rounded-lg border px-3 py-2 text-[10px] font-bold ${rankingMode === mode ? 'border-indigo-500/40 bg-indigo-500/15 text-indigo-200' : 'border-slate-700 bg-slate-950 text-slate-400'}`}>{modeLabel(mode)}</button>)}</div></div>

      <div className="mt-3 flex flex-wrap items-center gap-2 text-[10px]"><span className="rounded-full border border-emerald-500/20 bg-emerald-500/5 px-3 py-1 text-emerald-200">Producción: {scan.accepted} válidos</span><span className="rounded-full border border-cyan-500/20 bg-cyan-500/5 px-3 py-1 text-cyan-200">Externos: {externalLoading ? 'cargando…' : externalScan ? `${externalAccepted} válidos` : 'sin cargar'}</span><span className="rounded-full border border-violet-500/20 bg-violet-500/5 px-3 py-1 text-violet-200">Radar mostrado: {ranking.length}</span><button type="button" onClick={() => void loadExternalScan()} disabled={externalLoading || Boolean(externalScan)} className="flex items-center gap-1 rounded-lg border border-violet-500/30 px-3 py-1 font-bold text-violet-200 disabled:opacity-50"><RefreshCw className={`h-3 w-3 ${externalLoading ? 'animate-spin' : ''}`}/>{externalScan ? 'Externos cargados' : externalLoading ? 'Cargando externos…' : 'Cargar validación externa'}</button></div>
      {externalError && <div className="mt-2 rounded-lg border border-amber-500/20 bg-amber-500/5 p-2 text-[10px] text-amber-100">El universo externo no pudo cargarse completo: {externalError}. El analizador individual sigue disponible. <button type="button" onClick={() => { setExternalScan(null); void loadExternalScan(); }} className="ml-2 font-bold underline">Reintentar</button></div>}
      ...
      <div className="mt-3 overflow-x-auto rounded-xl border border-slate-800"><table className="w-full min-w-[820px] text-xs"><thead className="bg-slate-950 text-slate-500"><tr><th className="p-3 text-left">#</th><th className="p-3 text-left">Valor</th><th className="p-3 text-left">Uso</th><th className="p-3 text-left">Categoría</th><th className="p-3 text-right">Mom. 120d</th><th className="p-3 text-right">Vol.</th><th className="p-3 text-right">DD máx.</th><th className="p-3 text-right">Score</th><th className="p-3"></th></tr></thead><tbody>{visible.map((c, i) => {
        ...
        return <tr key={c.asset.assetId} className="border-t border-slate-800"><td className="p-3 text-slate-500">{i + 1}</td><td className="p-3"><b className="font-mono text-white">{c.asset.ticker}</b><div className="max-w-[240px] truncate text-[9px] text-slate-500">{c.asset.name}</div></td><td className="p-3"><span className={`rounded-full border px-2 py-1 text-[8px] font-black ${production ? 'border-emerald-500/30 bg-emerald-500/10 text-emerald-200' : 'border-violet-500/30 bg-violet-500/10 text-violet-200'}`}>{production ? 'CARTERA' : 'VALIDACIÓN EXTERNA'}</span></td><td className="p-3 text-slate-400">{c.asset.category}</td><td className={`p-3 text-right font-mono ${(c.momentum120Pct ?? 0) >= 0 ? 'text-emerald-300' : 'text-rose-300'}`}>{c.momentum120Pct?.toFixed(1) ?? 'N/D'}%</td><td className="p-3 text-right font-mono">{c.annualizedVolatilityPct?.toFixed(1) ?? 'N/D'}%</td><td className="p-3 text-right font-mono text-amber-200">{c.maxDrawdownPct?.toFixed(1) ?? 'N/D'}%</td><td className="p-3 text-right font-mono">{c.score?.toFixed(2) ?? 'N/D'}</td><td className="p-3 text-right"><button onClick={() => setSelectedSymbol(c.asset.ticker)} className="rounded-lg border border-cyan-500/30 bg-cyan-500/5 px-3 py-1.5 text-[10px] font-bold text-cyan-200">Abrir gráfica</button></td></tr>;
      })}</tbody></table></div>
      ...
      <div className="mt-3 text-[10px] text-slate-500"><b className="text-emerald-300">CARTERA</b> puede competir en “Qué haría hoy”. <b className="text-violet-300">VALIDACIÓN EXTERNA</b> se mantiene fuera de producción. Cargarla es opcional y no debe penalizar el uso normal del buscador.</div>
    </section>
