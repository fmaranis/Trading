...
export const InvestmentResearchLab: React.FC<Props> = ({ scan, decision, requestedSymbol }) => {
  ...
  return <div className="space-y-5">
    ...
    <section className="rounded-2xl border border-slate-800 bg-slate-900 p-5">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div><div className="flex items-center gap-2"><BarChart3 className="h-5 w-5 text-indigo-300"/><h2 className="font-bold text-white">Radar actual ampliado</h2></div><p className="mt-1 text-xs text-slate-400">Combina los instrumentos válidos del universo de cartera con el universo externo de robustez. Cada fila indica si puede competir por tu cartera o si está reservada como validación externa.</p></div><div className="flex flex-wrap gap-2">{(['OPPORTUNITY','MOMENTUM','SAFETY','PUNISHED'] as RankingMode[]).map(mode => <button key={mode} onClick={() => setRankingMode(mode)} className={`rounded-lg border px-3 py-2 text-[10px] font-bold ${rankingMode === mode ? 'border-indigo-500/40 bg-indigo-500/15 text-indigo-200' : 'border-slate-700 bg-slate-950 text-slate-400'}`}>{modeLabel(mode)}</button>)}</div></div>
      ...
      <div className="mt-3 text-[10px] text-slate-500"><b className="text-emerald-300">CARTERA</b> puede competir en “Qué haría hoy”. <b className="text-violet-300">VALIDACIÓN EXTERNA</b> se mantiene fuera de producción para no usar casos de prueba como candidatos reales, aunque sus métricas puedan ser muy altas. Cualquier valor sigue pudiendo estudiarse individualmente.</div>
    </section>
