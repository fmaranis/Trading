...
import { BarChart3, CircleDollarSign, Radar, RefreshCw, ShieldCheck, WalletCards } from 'lucide-react';
import {
...
function cashRole(result: InvestmentDecisionResult, eligibleCount: number | null): { label: string; reason: string; className: string } {
  if (result.cashWeight >= 0.999999) {
    return {
      label: 'CASH PRIORITARIO',
      reason: 'El motor no encuentra una asignación de riesgo que justifique sacar dinero de la cuenta remunerada. Mantener 100% cash es una decisión explícita, no un fallo de asignación.',
      className: 'border-sky-500/35 bg-sky-500/10 text-sky-100'
    };
  }
  if (result.marketRegime === 'BEAR_HIGH_VOL' || result.marketRegime === 'BEAR_LOW_VOL' || result.marketRegime === 'SIDEWAYS_HIGH_VOL') {
    return {
      label: 'REFUGIO DEFENSIVO',
      reason: `El régimen ${result.marketRegime} eleva deliberadamente el peso de efectivo. El cash compite con los activos, cobra la TAE configurada y reduce exposición mientras el entorno sea adverso.`,
      className: 'border-cyan-500/35 bg-cyan-500/10 text-cyan-100'
    };
  }
  if (eligibleCount === 0) {
    return {
      label: 'REFUGIO POR FALTA DE OPORTUNIDAD',
      reason: 'Ningún candidato supera simultáneamente la cuenta remunerada y el consenso mínimo. No invertir el capital disponible es, por tanto, la alternativa económica preferida hoy.',
      className: 'border-amber-500/35 bg-amber-500/10 text-amber-100'
    };
  }
  if (result.cashWeight >= 0.12) {
    return {
      label: 'RESERVA ESTRATÉGICA',
      reason: 'El perfil de riesgo y el régimen mantienen una reserva deliberada en efectivo aunque existan oportunidades. Esa reserva sigue remunerada y disponible para futuras entradas.',
      className: 'border-emerald-500/30 bg-emerald-500/10 text-emerald-100'
    };
  }
  return {
    label: 'BUFFER OPERATIVO',
    reason: 'El motor mantiene una fracción pequeña de efectivo por disciplina de cartera. No se interpreta como dinero ocioso: sigue remunerado y conserva opcionalidad para nuevas entradas.',
...
export const InteractiveInvestmentDecisionCenter: React.FC = () => {
  ...
  const runSecondaryValidation = async () => {
    ...
    if (!payload.length) return;
    setEodhdLoading(true); setEodhdError(null); setAlphaValidation(null); setAlphaError(null);
    try {
  ...
  return <div className="space-y-5">
    ...
    {workspace === 'PORTFOLIO' && <>
      ...
      {scan && result && positionHealthLoading && <div className="rounded-2xl border border-cyan-500/20 bg-cyan-500/5 p-4 text-sm text-cyan-100"><b>Finalizando la decisión…</b><div className="mt-1 text-[11px] text-slate-400">La oportunidad y el sizing ya están calculados, pero la pantalla no mostrará órdenes hasta reconciliar la salud de las posiciones actuales.</div></div>}

      {scan && result && !positionHealthLoading && positionHealth == null && <div className="rounded-2xl border border-rose-500/30 bg-rose-500/10 p-4 text-sm text-rose-100"><b>Decisión operativa bloqueada.</b><div className="mt-1 text-[11px] text-slate-300">No se ha podido cerrar la evaluación de salud de la cartera. Se conservan los datos de mercado, pero no se muestran compras, ventas ni rotaciones como decisión final.</div></div>}

      {scan && result && !positionHealthLoading && positionHealth != null && <MarketUtilityDashboard scan={scan} decision={result} eodhdValidation={eodhdValidation} positionHealth={positionHealth} onInspectAsset={inspectAsset}/>}

      {result && cashDecision && <details className={`rounded-2xl border p-4 ${cashDecision.className}`}>
        <summary className="touch-target flex cursor-pointer list-none items-center justify-between gap-3"><div className="flex items-center gap-2"><CircleDollarSign className="h-5 w-5"/><div><div className="font-bold">Papel del cash · {cashDecision.label}</div><div className="mt-0.5 text-[10px] opacity-70">Explicación secundaria de la misma decisión; no sustituye la orden principal.</div></div></div><span className="rounded-full border border-current/20 px-3 py-1 text-[9px] font-black">{pct(result.cashWeight)}</span></summary>
        <div className="mt-3"><p className="text-xs opacity-80">{cashDecision.reason}</p><div className="mt-3 grid gap-2 grid-cols-2 lg:grid-cols-4 text-xs"><div className="rounded-xl bg-slate-950/45 p-3"><div className="text-[9px] uppercase opacity-60">Objetivo cash</div><div className="mt-1 font-mono text-lg font-bold">{result.cashAmountEur.toFixed(2)} €</div></div><div className="rounded-xl bg-slate-950/45 p-3"><div className="text-[9px] uppercase opacity-60">Peso teórico</div><div className="mt-1 font-mono text-lg font-bold">{pct(result.cashWeight)}</div></div><div className="rounded-xl bg-slate-950/45 p-3"><div className="text-[9px] uppercase opacity-60">Remuneración</div><div className="mt-1 font-mono text-lg font-bold">{cashBenchmarkAnnualPct.toFixed(2)}% TAE</div></div><div className="rounded-xl bg-slate-950/45 p-3"><div className="text-[9px] uppercase opacity-60">Interés 1 año*</div><div className="mt-1 font-mono text-lg font-bold">+{oneYearCashInterestEur.toFixed(2)} €</div></div></div><div className="mt-2 text-[9px] opacity-65">* Equivalencia estática sobre el cash teórico si permaneciera inmóvil durante un año; no es una previsión de la cartera real.</div></div>
      </details>}

      {scan && result && <details className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4">
        <summary className="touch-target flex cursor-pointer items-center font-bold text-slate-200">Datos y controles técnicos de la decisión</summary>
        <div className="mt-4 space-y-4">
          <div className="flex flex-wrap items-center gap-2 text-xs text-slate-400"><span className={`rounded-lg border px-3 py-2 font-bold ${confidenceClass(result.confidence)}`}>Calidad de datos {result.confidence} · {result.confidenceScore}/100</span><span>Mide frescura, profundidad, cobertura común y régimen. No es probabilidad de beneficio ni convicción de una compra.</span></div>
          {result.dataQualityDiagnostics && <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4 text-[10px]">
            <div className="rounded-lg border border-slate-800 bg-slate-950 p-3"><div className="uppercase text-slate-500">Retraso mercado</div><div className="mt-1 font-mono font-bold text-white">{result.dataQualityDiagnostics.marketSessionAge} sesiones</div></div>
            <div className="rounded-lg border border-slate-800 bg-slate-950 p-3"><div className="uppercase text-slate-500">Histórico mínimo</div><div className="mt-1 font-mono font-bold text-white">{result.dataQualityDiagnostics.minimumAssetBars} barras</div></div>
            <div className="rounded-lg border border-slate-800 bg-slate-950 p-3"><div className="uppercase text-slate-500">Cobertura común</div><div className="mt-1 font-mono font-bold text-white">{result.dataQualityDiagnostics.commonCoveragePct.toFixed(1)}%</div></div>
            <div className="rounded-lg border border-slate-800 bg-slate-950 p-3"><div className="uppercase text-slate-500">Régimen</div><div className="mt-1 font-bold text-white">{result.dataQualityDiagnostics.regimeClassified ? 'Clasificado' : 'No clasificable'}</div></div>
          </div>}
