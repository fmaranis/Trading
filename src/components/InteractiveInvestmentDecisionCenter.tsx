...
export const InteractiveInvestmentDecisionCenter: React.FC = () => {
  ...
  return <div className="space-y-5">
    ...
    {result&&<><section className="grid gap-3 md:grid-cols-4"><div className="rounded-xl border border-slate-800 bg-slate-900 p-4"><Clock3 className="h-4 w-4 text-sky-400"/><div className="mt-2 text-[10px] uppercase text-slate-500">Datos hasta</div><div className="font-mono font-bold">{result.asOfDate}</div></div><div className="rounded-xl border border-slate-800 bg-slate-900 p-4"><BarChart3 className="h-4 w-4 text-indigo-400"/><div className="mt-2 text-[10px] uppercase text-slate-500">Régimen</div><div className="font-bold text-sm">{result.marketRegime}</div></div><div className="rounded-xl border border-slate-800 bg-slate-900 p-4"><ShieldCheck className="h-4 w-4 text-emerald-400"/><div className="mt-2 text-[10px] uppercase text-slate-500">Método</div><div className="font-bold text-sm">{result.recommendedMethod}</div></div><div className="rounded-xl border border-slate-800 bg-slate-900 p-4"><WalletCards className="h-4 w-4 text-amber-400"/><div className="mt-2 text-[10px] uppercase text-slate-500">Efectivo objetivo</div><div className="font-mono font-bold">{result.cashAmountEur.toFixed(2)} €</div><div className="text-xs text-slate-500">{pct(result.cashWeight)}</div></div></section>
      <section className="rounded-2xl border border-emerald-500/25 bg-emerald-950/20 p-5"><h2 className="font-bold text-emerald-100">Propuesta conjunta actual</h2><p className="mt-1 text-sm text-slate-200">{result.summary}</p><div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{result.assets.filter(a=>a.amountEur>=0.01).map(asset=>{const c=selectedById.get(asset.assetId);const price=c?.lastClose??0;const units=price>0?asset.amountEur/price:null;const isFund=c?.asset.instrumentType==='MUTUAL_FUND';return <div key={asset.assetId} className="rounded-xl border border-slate-700 bg-slate-950/80 p-4"><div className="flex items-center justify-between gap-2"><div className="font-bold">{asset.ticker}</div><span className="rounded border border-slate-700 px-1.5 py-0.5 text-[9px] text-slate-400">{isFund?'FONDO':'ETF/ETC'}</span></div><div className="text-[11px] text-slate-400">{asset.name}</div><div className="mt-3 flex justify-between"><span className="font-mono text-xl font-bold">{asset.amountEur.toFixed(2)} €</span><span className="text-indigo-300">{pct(asset.weight)}</span></div><div className="mt-1 text-[10px] text-slate-500">{isFund?'VL':'Cierre'} {price?`${price.toFixed(4)} €`:'N/D'} · {units==null?'N/D':`${units.toFixed(4)} ${isFund?'participaciones teóricas':'títulos teóricos'}`}</div></div>})}<div className="rounded-xl border border-slate-700 bg-slate-950/80 p-4"><div className="font-bold">EFECTIVO</div><div className="mt-3 font-mono text-xl font-bold">{result.cashAmountEur.toFixed(2)} €</div></div></div><div className="mt-3 text-xs text-slate-500">Asignado {allocated.toFixed(2)} € · total {(allocated+result.cashAmountEur).toFixed(2)} €</div></section>
