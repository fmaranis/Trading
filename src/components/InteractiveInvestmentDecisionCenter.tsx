...
function cashOnlyDecision(scan: AssetUniverseScanResult, capitalEur: number, riskProfile: InvestorRiskProfile, horizonYears: InvestmentHorizonYears): InvestmentDecisionResult {
  ...
  return {
    ...
    regimeTrendPct: null,
    regimeVolatilityPct: null,
    confidence: 'MEDIUM',
    confidenceScore: 70,
    recommendedMethod: methodFor(riskProfile),
    cashWeight: 1,
    cashAmountEur: capitalEur,
    assets: [],
    portfolioDatasetFingerprint: `CASH_ONLY:${asOfDate}`,
    evidence: 'REAL_ONLY',
  ...
  return <div className="space-y-5">
    ...
    {workspace === 'PORTFOLIO' && <>
      <section className="rounded-2xl border border-emerald-500/25 bg-gradient-to-br from-emerald-950/35 via-slate-900 to-slate-950 p-5 sm:p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div className="max-w-3xl"><div className="flex items-center gap-2"><WalletCards className="h-5 w-5 text-emerald-300"/><h1 className="text-xl sm:text-2xl font-bold text-white">Mi cartera real</h1></div><p className="mt-2 text-sm text-slate-300">Las nuevas compras compiten dentro de un universo EUR ampliado y deben superar primero tu alternativa de efectivo y el consenso. Tus posiciones reales se vigilan una a una.</p></div>{result && <div className={`rounded-xl border px-3 py-2 text-xs font-bold ${confidenceClass(result.confidence)}`}>Evidencia {result.confidence} · {result.confidenceScore}/100</div>}</div>
