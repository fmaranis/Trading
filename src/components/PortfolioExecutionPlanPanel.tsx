...
export const PortfolioExecutionPlanPanel: React.FC<Props> = ({ scan, decision }) => {
  ...
  return <section id="pending-portfolio-operations" className="rounded-2xl border border-cyan-500/25 bg-cyan-500/5 p-5">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <div><div className="flex items-center gap-2"><ClipboardList className="h-5 w-5 text-cyan-300"/><h2 className="font-bold">Operaciones pendientes · Mi cartera</h2></div><p className="mt-1 max-w-3xl text-[11px] text-slate-400">Convierte la recomendación en pasos manuales concretos. Para compras, suscripciones y traspasos, puedes confirmar tú mismo si el producto aparece en MyInvestor; la app conservará esa confirmación para futuras recomendaciones.</p></div>
    ...
    {plan && <>
      ...
      <div className="mt-4 rounded-lg border border-amber-500/20 bg-amber-500/5 p-3 text-[10px] text-amber-100">Las confirmaciones manuales de disponibilidad se guardan en este navegador y se reutilizan por ISIN/ticker. No sustituyen la comprobación de precio, comisión, clase exacta del fondo ni elegibilidad fiscal/operativa del traspaso.</div>
    </>}
  </section>;
};
