...
export const PortfolioExecutionPlanPanel: React.FC<Props> = ({ scan, decision, positionHealth, onInspectAsset }) => {
  ...
  return <section id="pending-portfolio-operations" className="rounded-2xl border border-cyan-500/25 bg-cyan-500/5 p-5">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <div><div className="flex items-center gap-2"><ClipboardList className="h-5 w-5 text-cyan-300"/><h2 className="font-bold">Qué haría hoy</h2></div><p className="mt-1 max-w-3xl text-[11px] text-slate-400">La operación final usa las mismas oportunidades actuales que la alerta superior y aplica capital disponible + concentración + costes + fiscalidad. Si hay una entrada financiable, este bloque no puede responder simplemente “mantener”.</p></div>
