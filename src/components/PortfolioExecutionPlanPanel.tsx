...
export const PortfolioExecutionPlanPanel: React.FC<Props> = ({ scan, decision }) => {
  ...
  return <section id="pending-portfolio-operations" className="rounded-2xl border border-cyan-500/25 bg-cyan-500/5 p-5">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <div><div className="flex items-center gap-2"><ClipboardList className="h-5 w-5 text-cyan-300"/><h2 className="font-bold">Operaciones pendientes · Mi cartera</h2></div><p className="mt-1 max-w-3xl text-[11px] text-slate-400">Aquí se aplican de verdad los filtros mostrados arriba: rentabilidad frente a efectivo, costes, títulos enteros y disponibilidad del producto.</p></div>
    ...
    {!plan && <div className="mt-4 rounded-xl border border-dashed border-slate-700 p-4 text-xs text-slate-500">Pulsa <b>Preparar operaciones</b>. El plan leerá la rentabilidad de la cuenta configurada en el bloque verde superior.</div>}

    {plan && <>
      <div className="mt-4 flex flex-wrap gap-2 text-[10px] text-slate-400"><span className="rounded-full border border-slate-700 px-3 py-1">Decisión: {plan.decisionAsOf}</span><span className="rounded-full border border-cyan-500/25 px-3 py-1 text-cyan-300">{pending} pendientes</span><span className="rounded-full border border-slate-700 px-3 py-1">Creado: {new Date(plan.createdAt).toLocaleString('es-ES')}</span><span className="rounded-full border border-emerald-500/25 px-3 py-1 text-emerald-200">Cuenta usada: {plan.cashBenchmarkAnnualPct.toFixed(2)}%</span><span className="rounded-full border border-amber-500/25 px-3 py-1 text-amber-200">Costes activos</span></div>
      ...
      <div className="mt-4 rounded-lg border border-amber-500/20 bg-amber-500/5 p-3 text-[10px] text-amber-100">El proxy anual neto es un filtro histórico, no una previsión. Una operación solo aparece como compra/suscripción si supera la cuenta remunerada y los demás gates de ejecución.</div>
    </>}
  </section>;
};
