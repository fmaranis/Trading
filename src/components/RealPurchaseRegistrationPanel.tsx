...
export const RealPurchaseRegistrationPanel: React.FC<Props> = ({ scan, decision, portfolioDecision }) => {
  ...
  }, [scan, decision.asOfDate, portfolioDecision]);
  ...
  return <section id="register-real-purchase" className="rounded-2xl border border-emerald-500/25 bg-emerald-500/5 p-4 sm:p-5">
    <div className="flex items-start gap-2"><WalletCards className="mt-0.5 h-5 w-5 text-emerald-300"/><div><h2 className="font-bold text-white">Registrar compra ejecutada en mi cartera</h2><p className="mt-1 text-[11px] text-slate-400">Estas líneas se derivan de la misma decisión canónica mostrada arriba. Cuando MyInvestor confirme la ejecución, corrige aquí el importe real; para acciones/ETF también los títulos y la comisión.</p></div></div>
