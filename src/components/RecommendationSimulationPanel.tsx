...
function simulateSnapshot(snapshot: MarketSnapshotEntry, scan: AssetUniverseScanResult, capitalEur: number) {
  ...
  return {
    ...
    strategicCashFinal,
    finalEur,
    returnPct: (finalEur / capitalEur - 1) * 100,
    allCashFinalEur: allCash.finalEur,
    allCashReturnPct: allCash.returnPct,
    excessEur: finalEur - allCash.finalEur,
    excessPctPoints: (finalEur / capitalEur - 1) * 100 - allCash.returnPct
  };
}

export const RecommendationSimulationPanel: React.FC<Props> = ({ scan }) => {
  const snapshots = useMemo(() => MarketSnapshotHistoryService.load(), [scan]);
  ...
  return <section className="rounded-2xl border border-fuchsia-500/25 bg-fuchsia-500/5 p-5">
    <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
      <div><div className="flex items-center gap-2"><FlaskConical className="h-5 w-5 text-fuchsia-300"/><h2 className="font-bold">Si hubiera seguido una recomendación anterior…</h2></div><p className="mt-1 max-w-3xl text-[11px] text-slate-400">Simulación sobre las recomendaciones que la propia app guardó. Usa el primer precio ejecutable posterior a la decisión, títulos enteros y comisión para ETF/ETC, participaciones fraccionarias para fondos y remunera el efectivo restante con la misma referencia configurada.</p></div>
    ...
    {!selected && <div className="mt-4 rounded-xl border border-dashed border-slate-700 p-4 text-xs text-slate-500">Aún no hay recomendaciones guardadas en este navegador. Desde ahora la pantalla principal guarda una snapshot diaria automáticamente.</div>}

    {selected && simulation && <>
      <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-5 text-xs">
        <div className="rounded-xl bg-slate-950 p-3"><div className="text-[9px] uppercase text-slate-500">Recomendación</div><b>{selected.asOfDate}</b><div className="text-[10px] text-slate-500">hasta {simulation.endDate}</div></div>
        <div className="rounded-xl bg-slate-950 p-3"><div className="text-[9px] uppercase text-slate-500">Valor hoy</div><b className="font-mono">{simulation.finalEur.toFixed(2)} €</b><div className={simulation.returnPct >= 0 ? 'text-emerald-300' : 'text-rose-300'}>{simulation.returnPct >= 0 ? '+' : ''}{simulation.returnPct.toFixed(2)}%</div></div>
        <div className="rounded-xl bg-slate-950 p-3"><div className="text-[9px] uppercase text-slate-500">Si lo hubiera dejado en cuenta</div><b className="font-mono">{simulation.allCashFinalEur.toFixed(2)} €</b><div className="text-slate-500">+{simulation.allCashReturnPct.toFixed(2)}%</div></div>
        <div className={`rounded-xl p-3 ${simulation.excessEur >= 0 ? 'bg-emerald-500/10' : 'bg-amber-500/10'}`}><div className="text-[9px] uppercase text-slate-500">Valor añadido</div><b className={`font-mono ${simulation.excessEur >= 0 ? 'text-emerald-200' : 'text-amber-200'}`}>{simulation.excessEur >= 0 ? '+' : ''}{simulation.excessEur.toFixed(2)} €</b><div className="text-slate-500">{simulation.excessPctPoints >= 0 ? '+' : ''}{simulation.excessPctPoints.toFixed(2)} pp</div></div>
        <div className="rounded-xl bg-slate-950 p-3"><div className="text-[9px] uppercase text-slate-500">Resultado</div><div className="mt-1 flex items-center gap-1 font-bold">{simulation.excessEur >= 0 ? <><TrendingUp className="h-4 w-4 text-emerald-300"/><span className="text-emerald-200">MEJOR QUE EFECTIVO</span></> : <><TrendingDown className="h-4 w-4 text-amber-300"/><span className="text-amber-200">PEOR QUE EFECTIVO</span></>}</div></div>
      </div>
      ...
      <div className="mt-3 text-[10px] text-slate-500">Es una simulación histórica de una recomendación guardada, no prueba de que una recomendación futura vaya a comportarse igual. No modela fiscalidad, spread ni settlement de fondos.</div>
    </>}
  </section>;
};