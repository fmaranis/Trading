...
export const CurrentOpportunityAlertsPanel: React.FC<Props> = ({ scan, decision, onInspectAsset }) => {
  ...
  return <section className={`rounded-2xl border p-5 ${high.length > 0 ? 'border-emerald-400/35 bg-emerald-500/5' : 'border-amber-500/20 bg-slate-900'}`}>
    <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
      <div>
        <div className="flex items-center gap-2">{high.length > 0 ? <Sparkles className="h-5 w-5 text-emerald-300"/> : <BellRing className="h-5 w-5 text-amber-300"/>}<h2 className="text-lg font-bold text-white">Alertas importantes de entrada ahora</h2></div>
        <p className="mt-1 max-w-3xl text-xs text-slate-400">Solo aparecen activos que ya superan datos REAL + cuenta remunerada + consenso BUY + ausencia de tendencia estructural bajista. Alta convicción exige además acuerdo fuerte entre señales, momentum alineado y riesgo razonable.</p>
      </div>
      <div className={`rounded-xl border px-4 py-2 text-xs font-black ${high.length > 0 ? 'border-emerald-400/35 bg-emerald-500/10 text-emerald-200' : 'border-amber-500/25 bg-amber-500/5 text-amber-200'}`}>{high.length > 0 ? `${high.length} ALTA CONVICCIÓN` : 'SIN ALTA CONVICCIÓN HOY'}</div>
    </div>

    {visible.length > 0 ? <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{visible.map((alert, index) => <article key={alert.assetId} className={`rounded-xl border p-4 ${levelClass(alert.level)}`}>
      ...
      <details className="mt-3 text-[10px]"><summary className="cursor-pointer font-bold opacity-80">Por qué</summary><div className="mt-2 space-y-1 opacity-70">{alert.reasons.map(reason => <div key={reason}>• {reason}</div>)}</div></details>
    ...
    </article>)}</div> : <div className="mt-4 rounded-xl border border-dashed border-slate-700 bg-slate-950/50 p-4 text-sm text-slate-400">No hay ninguna entrada que supere hoy todos los filtros mínimos. La ausencia de alerta es una señal válida: no se fuerza una compra.</div>}

    {alerts.length > visible.length && <div className="mt-3 text-[10px] text-slate-500">Se muestran las 6 mejores entradas actuales de {alerts.length} válidas. El resto sigue disponible en “Qué haría hoy” y en el radar.</div>}
