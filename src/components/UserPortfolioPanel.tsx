  ...
  return <section className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-5">
    ...
    <div className="mt-4 grid gap-3 sm:grid-cols-3">
      <div className="rounded-xl border border-slate-800 bg-slate-950 p-4">
        <div className="text-[10px] uppercase text-slate-500">Invertido ahora</div>
        <div className="mt-1 font-mono text-xl font-black text-white">{investedEur.toFixed(2)} €</div>
        <div className="mt-1 text-[10px] text-slate-500">Fondos {fundRegisteredValue.toFixed(2)} € · ETFs/ETCs {listedHoldingsValue.toFixed(2)} €</div>
      </div>
      <div className="rounded-xl border border-cyan-500/25 bg-cyan-500/5 p-4">
        <div className="text-[10px] uppercase text-cyan-300">Liquidez disponible</div>
        <div className="mt-1 font-mono text-xl font-black text-cyan-100">{liquidityEur.toFixed(2)} €</div>
        <div className="mt-1 text-[10px] text-slate-500">Efectivo {cash.toFixed(2)} € + capital pendiente {plan.availableEur.toFixed(2)} €</div>
      </div>
      <div className="rounded-xl border border-violet-500/25 bg-violet-500/5 p-4">
        <div className="text-[10px] uppercase text-violet-300">Capital total controlado</div>
        <div className="mt-1 font-mono text-xl font-black text-violet-100">{totalEur.toFixed(2)} €</div>
        <div className="mt-1 text-[10px] text-slate-500">Invertido + liquidez. Es la referencia económica única de esta pantalla.</div>
      </div>
    </div>
    ...
    <details className="mt-4 rounded-xl border border-slate-800 bg-slate-950/40 p-4">
      <summary className="flex cursor-pointer list-none items-center justify-between gap-3"><div><b>Editar cartera y liquidez</b><div className="mt-1 text-[10px] text-slate-500">Solo para reconciliar manualmente con tu cuenta real.</div></div><ChevronDown className="h-4 w-4 text-slate-500"/></summary>
    ...
    <details className="mt-3 rounded-xl border border-violet-500/20 bg-violet-500/5 p-4">
      <summary className="flex cursor-pointer list-none items-center justify-between gap-3"><div><b>Ver diagnóstico teórico</b><div className="mt-1 text-[10px] text-slate-500">Solo explica pesos y desviaciones. No es otra recomendación operativa.</div></div><ChevronDown className="h-4 w-4 text-slate-500"/></summary>
      <div className="mt-4 border-t border-violet-500/15 pt-4">
        ...
        <div className="mt-3 text-[10px] text-slate-500">Capital teóricamente asignable: {portfolioDecision.recommendedNewInvestmentEur.toFixed(2)} € · objetivo/residual de cash: {portfolioDecision.residualPlannedCashEur.toFixed(2)} €. La acción ejecutable final se decide únicamente en “Qué haría hoy”.</div>
