...
export const UserPortfolioPanel: React.FC<Props> = ({ scan, decision }) => {
  ...
  return <section className="space-y-4 rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-4">
    <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
      <div><div className="flex items-center gap-2"><WalletCards className="h-4 w-4 text-emerald-300"/><h3 className="font-bold">Mi cartera real</h3></div><p className="mt-1 text-[11px] text-slate-400">Aquí solo se registra lo que ya tienes y el capital pendiente. El histórico, ranking y motivos de todos los fondos/ETFs están en una única zona común más abajo.</p></div>
    ...
    <div className="rounded-xl border border-violet-500/25 bg-violet-500/5 p-4">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div><b>Qué hacer con tu cartera y el dinero nuevo</b><div className="mt-1 text-[11px] text-slate-400">Una sola decisión consolidada: descuenta primero lo que ya posees y solo después calcula los déficits a cubrir.</div></div><div className="rounded-lg border border-violet-500/30 px-3 py-2 text-xs text-violet-200">Aportación nueva propuesta: <b>{portfolioDecision.recommendedNewInvestmentEur.toFixed(2)} €</b> · efectivo reservado: <b>{portfolioDecision.residualPlannedCashEur.toFixed(2)} €</b></div></div>

      <div className="mt-3 grid gap-2 md:grid-cols-2 xl:grid-cols-4 text-xs">{portfolioDecision.exposures.map(x => <div key={x.category} className="rounded-lg border border-slate-800 bg-slate-950 p-3"><div className="text-[10px] uppercase text-slate-500">{x.category}</div><div className="mt-1 flex justify-between"><span>Actual</span><b>{x.currentValueEur.toFixed(2)} €</b></div><div className="flex justify-between text-slate-400"><span>Objetivo</span><span>{x.targetValueEur.toFixed(2)} €</span></div><div className={`mt-1 text-right font-mono ${x.gapEur > 0 ? 'text-emerald-300' : x.gapEur < 0 ? 'text-amber-300' : 'text-slate-500'}`}>{x.gapEur >= 0 ? '+' : ''}{x.gapEur.toFixed(2)} €</div></div>)}</div>

      <div className="mt-4 grid gap-3 lg:grid-cols-2">
        <div className="rounded-xl border border-slate-800 bg-slate-950/60 p-3"><div className="text-xs font-bold">Posiciones actuales</div><div className="mt-2 space-y-2">{portfolioDecision.existingPositions.length === 0 && <div className="text-xs text-slate-600">No hay posiciones existentes.</div>}{portfolioDecision.existingPositions.map(x => <div key={`${x.instrumentType}_${x.id}`} className="rounded-lg border border-slate-800 p-2 text-xs"><div className="flex items-start justify-between gap-2"><div><b>{x.label}</b><div className="text-[10px] text-slate-500">{x.instrumentType === 'MUTUAL_FUND' ? 'Fondo' : 'ETF/ETC'} · {x.category} · {x.currentValueEur == null ? 'N/D' : `${x.currentValueEur.toFixed(2)} €`}</div></div><b className={portfolioActionClass(x.action)}>{portfolioActionLabel(x.action)}</b></div><div className="mt-1 text-[10px] text-slate-500">{x.reason}</div></div>)}</div></div>
        <div className="rounded-xl border border-slate-800 bg-slate-950/60 p-3"><div className="text-xs font-bold">Capital nuevo</div><div className="mt-2 space-y-2">{portfolioDecision.contributions.length === 0 && <div className="text-xs text-slate-600">No hay aportaciones nuevas calculables con los datos actuales.</div>}{portfolioDecision.contributions.map(x => <div key={x.assetId} className="rounded-lg border border-slate-800 p-2 text-xs"><div className="flex items-start justify-between gap-2"><div><b>{x.ticker}</b><div className="text-[10px] text-slate-500">{x.name} · {x.instrumentType === 'MUTUAL_FUND' ? 'FONDO' : 'ETF/ETC'} · {x.category}</div></div><b className="font-mono text-emerald-300">{x.amountEur.toFixed(2)} €</b></div></div>)}</div></div>
      </div>
