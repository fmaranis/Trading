...
export const UserPortfolioPanel: React.FC<Props> = ({ scan, portfolioDecision, positionHealth, onInspectAsset }) => {
  ...
  const [portfolioUpdatedAt, setPortfolioUpdatedAt] = useState(initial.updatedAt);
  ...
  const applyState = (next: UserPortfolioState) => {
    ...
    setFundMarketValues({});
    setPortfolioUpdatedAt(next.updatedAt);
  ...
  return <section className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-4 sm:p-5">
    <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
      ...
      <span className="rounded-full border border-emerald-500/30 bg-emerald-500/5 px-3 py-1 text-[10px] font-bold text-emerald-200">ESTADO PRIVADO · {portfolioUpdatedAt.slice(0, 10)}</span>
    </div>
    ...
    <div className="mt-4 rounded-xl border border-slate-800 bg-slate-950/60 p-4">
      ...
      <div className="mt-3 grid gap-2 md:grid-cols-2">
        ...
        {holdings.map(holding => {
          ...
          return <button type="button" key={holding.ticker} disabled={!onInspectAsset} onClick={() => onInspectAsset?.(holding.ticker)} className="touch-target rounded-lg border border-slate-800 bg-slate-900/60 p-3 text-left text-xs transition hover:border-cyan-500/40 hover:bg-slate-900 disabled:cursor-default disabled:hover:border-slate-800">
    ...
    <details className="mt-4 rounded-xl border border-slate-800 bg-slate-950/40 p-4">
      <summary className="touch-target flex cursor-pointer list-none items-center justify-between gap-3"><div><b>Editar cartera y liquidez</b><div className="mt-1 text-[10px] text-slate-500">Los cambios son un borrador local hasta pulsar Guardar reconciliación; no alteran la decisión superior antes de guardarse.</div></div><ChevronDown className="h-4 w-4 text-slate-500"/></summary>
      <div className="mt-4 border-t border-slate-800 pt-4">
        <div className="grid gap-3 lg:grid-cols-[0.34fr_0.66fr]">
          ...
          <div className="space-y-3">
            <div className="rounded-xl border border-slate-800 bg-slate-950/50 p-3"><div className="flex items-center justify-between"><div><b className="text-sm">Fondos</b><div className="text-[10px] text-slate-500">Edita únicamente los datos de esta cuenta.</div></div><button type="button" onClick={addFund} className="touch-target flex items-center gap-1 rounded-lg border border-dashed border-slate-700 px-2 py-1 text-[10px] text-slate-400"><Plus className="h-3 w-3"/>Fondo</button></div><div className="mt-2 space-y-3">{funds.length === 0 && <div className="rounded-lg border border-dashed border-slate-800 p-3 text-xs text-slate-600">Sin fondos registrados.</div>}{funds.map(f => <FundMarketDataCard key={f.id} fund={f} onChange={patch=>updateFund(f.id,patch)} onRemove={()=>removeFund(f.id)} onMarketValue={value=>setFundMarketValues(prev=>prev[f.id]===value?prev:{...prev,[f.id]:value})}/>)}</div></div>
            <div className="rounded-xl border border-slate-800 bg-slate-950/50 p-3"><div className="flex items-center justify-between"><div><b className="text-sm">Activos cotizados</b><div className="text-[10px] text-slate-500">Ticker libre; el datalist sólo ayuda, no limita.</div></div><button type="button" onClick={addHolding} className="touch-target flex items-center gap-1 rounded-lg border border-dashed border-slate-700 px-2 py-1 text-[10px] text-slate-400"><Plus className="h-3 w-3"/>Activo</button></div><datalist id="portfolio-known-tickers">{knownTickers.map(t => <option key={t} value={t}/>)}</datalist><div className="mt-2 space-y-2">{holdings.length === 0 && <div className="rounded-lg border border-dashed border-slate-800 p-3 text-xs text-slate-600">Sin activos cotizados registrados.</div>}{holdings.map((holding, index) => <div key={`${index}_${holding.ticker}`} className="grid grid-cols-[1fr_110px_44px] gap-2"><input list="portfolio-known-tickers" value={holding.ticker} onChange={e => updateHolding(index, { ticker: e.target.value.toUpperCase() })} placeholder="Ticker libre…" className="rounded-lg border border-slate-700 bg-slate-900 px-2 py-2 font-mono text-xs"/><input type="number" min="0" step="1" value={holding.shares} onChange={e => updateHolding(index, { shares: Math.max(0, Number(e.target.value) || 0) })} className="rounded-lg border border-slate-700 bg-slate-900 px-2 py-2 text-right font-mono text-xs" title="Títulos"/><button type="button" onClick={() => removeHolding(index)} className="touch-target rounded-lg border border-slate-700 text-slate-400 hover:text-rose-300" aria-label={`Eliminar ${holding.ticker || 'activo'}`}><Trash2 className="mx-auto h-3.5 w-3.5"/></button></div>)}</div></div>
          </div>
        </div>
      </div>
    </details>
    ...
    <details className="mt-3 rounded-xl border border-violet-500/20 bg-violet-500/5 p-4">
      <summary className="touch-target flex cursor-pointer list-none items-center justify-between gap-3"><div><b>Distribución de la decisión canónica · no es otra orden</b><div className="mt-1 text-[10px] text-slate-500">Se reutiliza exactamente el mismo `portfolioDecision` que alimenta la tarjeta superior. Los borradores sin guardar no lo recalculan.</div></div><ChevronDown className="h-4 w-4 text-slate-500"/></summary>
      <div className="mt-4 border-t border-violet-500/15 pt-4">
        ...
        <div className="mt-3 text-[10px] text-slate-500">Asignación teórica nueva: {portfolioDecision.recommendedNewInvestmentEur.toFixed(2)} € · cash teórico tras plan: {portfolioDecision.residualPlannedCashEur.toFixed(2)} €. La orden ejecutable se muestra arriba después de redondeo, costes, broker y fiscalidad.</div>
