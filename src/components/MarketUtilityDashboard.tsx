...
export const MarketUtilityDashboard: React.FC<Props> = ({ scan, decision, eodhdValidation, positionHealth, onInspectAsset }) => {
  ...
  useEffect(() => {
    ...
    const nextAlerts = OpportunityAlertEngine.evaluate({ scan, decision, previousDecision, evidence, cashBenchmarkAnnualPct: CashBenchmarkService.load() });
    ...
    <details className="rounded-2xl border border-violet-500/20 bg-slate-900 p-5">
      <summary className="cursor-pointer list-none">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div><div className="flex items-center gap-2"><Radar className="h-5 w-5 text-violet-300"/><h2 className="text-lg font-bold">Seguimiento de mi cartera</h2></div><p className="mt-1 text-xs text-slate-400">Alertas, snapshots y memoria de recomendaciones relacionadas con la decisión real. Las oportunidades históricas usan el mismo gate de cash + consenso que la cartera actual.</p></div>
