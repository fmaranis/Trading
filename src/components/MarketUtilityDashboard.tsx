...
export const MarketUtilityDashboard: React.FC<Props> = ({ scan, decision, eodhdValidation }) => {
  ...
  return <section className="space-y-4">
    ...
    <RecommendationSimulationPanel scan={scan} />

    <section className="rounded-2xl border border-violet-500/20 bg-slate-900 p-5">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div><div className="flex items-center gap-2"><Radar className="h-5 w-5 text-violet-300"/><h2 className="text-lg font-bold">Alertas y seguimiento</h2></div><p className="mt-1 text-xs text-slate-400">Cambios respecto a recomendaciones anteriores. Las operaciones actuales, la cartera y la simulación están arriba en el mismo flujo.</p></div>
        <div className="rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-xs text-slate-300"><History className="mr-1 inline h-3.5 w-3.5"/>{history.length} recomendaciones guardadas</div>
      </div>
    ...
    <details className="rounded-2xl border border-slate-800 bg-slate-900 p-5">
      <summary className="cursor-pointer font-bold">Historial de recomendaciones</summary>
      <div className="mt-2 text-[11px] text-slate-500">Estas snapshots alimentan el simulador de arriba. Se conserva el historial para poder comprobar después qué habría ocurrido si hubieras seguido cada recomendación.</div>
