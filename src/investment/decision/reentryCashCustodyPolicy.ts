...
export function effectiveReentryReservations(input: {
  ...
  const ordered = [...input.reservations]
  ...
  let remainingCapacity = Math.min(Math.max(0, input.baseDeployableToAssetsEur), cashAboveTarget);
  const out = new Map<string, number>();
  for (const reservation of ordered) {
    if (remainingCapacity <= 0.01) break;
    const amount = Math.min(reservation.amountEur, remainingCapacity);
...
export function availableCashRespectingReentryReservations(
  ...
  return Math.max(0, cashEur - Math.min(Math.max(0, cashEur), protectedOtherReservations));
}
