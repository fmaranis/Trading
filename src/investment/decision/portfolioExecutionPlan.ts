...
export interface PortfolioExecutionPlan { id: string; createdAt: string; decisionAsOf: string; lines: PortfolioExecutionLine[]; warnings: string[]; }
...
function candidatePrice(scan: AssetUniverseScanResult, ticker: string | undefined): number | null {
  if (!ticker) return null; const row = scan.candidates.find(c => c.asset.ticker.toUpperCase() === ticker.toUpperCase());
  ...
  for (const contribution of portfolioDecision.contributions) {
    ...
    if (!isFund && price) {
      ...
      if (shares < 1) {
        ...
        lines.push({ id: uid('review_buy', lines.length), action: 'REVIEW', status: 'PENDING', instrumentType: 'ETF_ETC', targetAssetId: contribution.assetId, targetTicker: contribution.ticker, targetName: contribution.name, targetIsin: asset?.isin, category: contribution.category, amountEur: amount, shares: null, estimatedPriceEur: price, estimatedFeeEur: null, instruction: `No comprar todavía: ${amount.toFixed(2)} € no alcanzan una participación completa de ${contribution.ticker}.`, rationale: contribution.reason });
        continue;
      }
      amount = shares * price;
      ...
      if (!gate.ok) {
        ...
        lines.push({ id: uid('review_cost_buy', lines.length), action: 'REVIEW', status: 'PENDING', instrumentType: 'ETF_ETC', targetAssetId: contribution.assetId, targetTicker: contribution.ticker, targetName: contribution.name, targetIsin: asset?.isin, category: contribution.category, amountEur: amount, shares, estimatedPriceEur: price, estimatedFeeEur: estimatedFee, instruction: `No comprar todavía ${contribution.ticker}: ${gate.reason}`, rationale: `${contribution.reason} La señal teórica se conserva; la ejecución se aplaza por coste.` });
        continue;
      }
    }
    lines.push({ id: uid(isFund ? 'subscribe' : 'buy', lines.length), action: isFund ? 'SUBSCRIBE_FUND' : 'BUY_ETF', status: 'PENDING', instrumentType: contribution.instrumentType, targetAssetId: contribution.assetId, targetTicker: contribution.ticker, targetName: contribution.name, targetIsin: asset?.isin, category: contribution.category, amountEur: amount, shares, estimatedPriceEur: price, estimatedFeeEur: estimatedFee, instruction: isFund ? `Suscribir aproximadamente ${amount.toFixed(2)} € del fondo ${contribution.name}${asset?.isin ? ` (ISIN ${asset.isin})` : ''}.` : `Comprar ${shares} participación${shares === 1 ? '' : 'es'} de ${contribution.ticker}${asset?.isin ? ` (ISIN ${asset.isin})` : ''}. Comisión estimada: ${estimatedFee?.toFixed(2) ?? 'N/D'} €.`, rationale: contribution.reason, taxNote: isFund ? 'Si el dinero procede de otro fondo elegible, revisar primero si conviene traspaso en vez de reembolso + suscripción.' : undefined });
  }
