 ...
 * trend-structure diagnostics. It keeps the current CORE_ARCHITECTURE_V1 fixed;
 * only the relative rank of already-eligible candidates gets a bounded
 * slope-quality adjustment.
 */
export function runDynamicReplayWithSlopeSelectionExperiment(input: ReplayRunInput): DynamicHistoricalReplayResult {
  ...
  try {
    ...
    result.notes.push(
      'SELECTION_SLOPE_V1: mismo replay CORE_ARCHITECTURE_V1 y mismo sizing; sólo cambia el ranking entre candidatos que ya han pasado REAL + cash + consenso BUY + Entry Timing.',
