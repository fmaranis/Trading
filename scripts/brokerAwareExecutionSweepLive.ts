...
async function main() {
  ...
  try {
    ...
    const selectedFundIds = new Set(research.selectionHistory.flatMap(x => x.selectedAssetIds));
    const fundEligibility = EUR_ASSET_UNIVERSE.filter(x => x.instrumentType === 'MUTUAL_FUND').map(fund => {
      ...
      const firstCausalEligibleDate = series.length >= CAUSAL_UNIVERSE_MINIMUM_HISTORY_BARS
        ? series[CAUSAL_UNIVERSE_MINIMUM_HISTORY_BARS - 1].timestamp.slice(0, 10)
        : null;
      ...
      return {
        ...
        ticker: fund.ticker,
        status: candidate?.status ?? 'NOT_SCANNED',
        bars: candidate?.bars ?? 0,
        currentScore: candidate?.score == null ? null : Number(candidate.score.toFixed(3)),
        currentShortlist: scan.selected.some(x => x.asset.assetId === fund.assetId),
        firstCausalEligibleDate,
        lastResearchInformationDate,
        eligibleBeforeLastMonthlyDecision,
        causalSelectionAppearances: appearances,
        diagnosis: candidate?.status !== 'ACCEPTED'
          ? `NOT_ACCEPTED:${candidate?.reason ?? 'UNKNOWN'}`
          : !eligibleBeforeLastMonthlyDecision
    ...
    const result = {
      ...
      fundEligibilitySummary: {
        acceptedFunds: fundEligibility.filter(x => x.status === 'ACCEPTED').length,
        currentlySelectedFunds: fundEligibility.filter(x => x.currentShortlist).length,
