...
async function main() {
  ...
  try {
    ...
    const sweep = capitals.map(initialCapital => {
      ...
      return {
        ...
        windowsFullySuppressed: replay.windowsFullySuppressed,
    ...
    const result = {
      ...
      scope: 'BROKER_AWARE_EXECUTION_CAPITAL_SWEEP',
      researchReference: {
        initialCapitalEur: research.initialCapital,
        researchTrades: research.totalTrades,
        researchRebalanceWindows: research.rebalanceCount,
        researchReturnPct: Number(research.totalReturnPct.toFixed(2))
      },
      policy: {
        ...
        wholeShares: true,
        fundWeightsHeldAsCash: true
      },
      sweep,
      interpretation: 'COMPARE_RESEARCH_SIGNAL_WITH_COST_AWARE_WHOLE_SHARE_EXECUTION',
      note: 'Fund target weights are conservatively held as cash in this ETF execution replay. Results are execution diagnostics, not profitability forecasts.'
    };
