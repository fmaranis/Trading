...
const PROTOCOL = {
  frozenAt: '2026-09-08',
  purpose: 'TEST_BOUNDED_QUALITY_SIGNAL_REACH_INSIDE_EXISTING_PORTFOLIO_ALLOCATOR',
  productionArchitecture: 'CORE_ARCHITECTURE_V1',
  productionPolicy: 'LEGACY',
  experimentalPolicy: 'QUALITY_ALLOCATION_BRIDGE_V1',
  onlyDifference: 'EXISTING_OPPORTUNITY_PRIORITY_MULTIPLIED_BY_FROZEN_QUALITY_V1_ADJUSTMENT_AS_PERCENT',
  qualityMultiplierFormula: 'clamp(1 + candidateQualityAdjustment(reliability, opportunity)/100, 0.85, 1.15)',
  gateSelectionPolicy: 'LEGACY_UNCHANGED',
  hardGatesUnchanged: true,
  cashConsensusTimingUnchanged: true,
...
function compareAcquisitionMaps(a: Map<string, Map<string, number>>, b: Map<string, Map<string, number>>) {
  ...
  return {
    changedDates: changedDates.length,
    assetSetChangedDates,
    amountOnlyChangedDates,
    totalAbsoluteAmountDeltaEur,
    maxDateAbsoluteAmountDeltaEur,
    changedDatesSample: changedDates.slice(0, 16)
  };
}
...
function compareTraces(legacy: AllocationTrace[], bridge: AllocationTrace[]) {
  ...
  return {
    decisionCountParityViolations,
    changedDecisionGates,
    assetSetChangedDecisionGates,
    amountOnlyChangedDecisionGates,
    totalAbsolutePlanDeltaEur,
    maxDecisionAbsolutePlanDeltaEur,
    changedDatesSample: changedDates.slice(0, 16)
  };
}
...
function bridgeMultiplierStats(traces: AllocationTrace[]) {
  ...
  const modulated = values.filter(value => Math.abs(value - 1) > 1e-9);
  return {
    observations: values.length,
    modulatedObservations: modulated.length,
...
async function main() {
  ...
  if (!(await waitForHealth(`${baseUrl}/api/health`, 1500))) {
    ...
    ownsServer = true;
    if (!(await waitForHealth(`${baseUrl}/api/health`, 30_000))) throw new Error('OPPORTUNITY_QUALITY_ALLOCATION_LOCAL_SERVER_UNAVAILABLE');
  }
  ...
  try {
    ...
    if (dataset.assets.length < MIN_ACCEPTED_ASSETS) throw new Error(`OPPORTUNITY_QUALITY_ALLOCATION_INSUFFICIENT_REAL_ASSETS:${dataset.assets.length}`);
    if (nonReal.length) throw new Error(`OPPORTUNITY_QUALITY_ALLOCATION_NON_REAL_DATA:${nonReal.map(asset => asset.assetId).join(',')}`);
    if (currentDiscoveryLeak.length) throw new Error(`OPPORTUNITY_QUALITY_ALLOCATION_CURRENT_DISCOVERY_LEAK:${currentDiscoveryLeak.map(asset => asset.assetId).join(',')}`);
    ...
    const result = {
      version: VERSION,
      status: 'PASS_HISTORICAL_ARCHITECTURE_DIAGNOSTIC_ONLY',
      ...
      interpretationContract: {
        ...
        historicalDiagnosticCanChangeProduction: false,
        consumedWindowsCannotBeUsedForTuning: true,
        bridgeFormulaFrozenBeforeThisRun: true,
        futureForwardRequiredBeforeAnyPromotion: true,
        slopeV1RetiredFromPromotionPath: true,
        retainedForwardRiskV8Finding: 'PREDICTIVE_DOWNSIDE_INFORMATION_RETAINED_BUT_NOT_USED_IN_THIS_BRIDGE'
      },
      notes: [
        'The bridge is implemented inside the existing PortfolioDecisionEngine as an optional research-only allocation policy; default and production remain LEGACY.',
        'The bridge reuses the already-frozen QUALITY_V1 reliability/opportunity adjustment and cannot change hard eligibility gates.',
        'The gate selection policy remains LEGACY in this diagnostic so the experiment isolates capital allocation reach rather than candidate-set changes.',
        'Starter/build, timing fractions, asset/category caps, portfolio slots, rotation rules, cash target, costs and taxes remain unchanged.',
        'These same historical windows are consumed architecture diagnostics and cannot authorize production promotion or coefficient tuning.'
      ]
    };
...
main().catch(error => {
  console.error('OPPORTUNITY_QUALITY_ALLOCATION_BRIDGE_FATAL', error);
  process.exitCode = 1;
});
