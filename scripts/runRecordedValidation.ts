...
async function main() {
  ...
  // Commit all result files generated by this validation in one isolated commit.
  if (records.length && process.env.VALIDATION_AUTO_COMMIT !== '0') {
    ...
    try {
      ...
      if (changed) {
        ...
        console.log(JSON.stringify({ committed: true, pushed, branch, commitSha, paths }, null, 2));
      }
    } catch (error: any) {
