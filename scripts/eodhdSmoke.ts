...
async function main() {
  ...
  try {
    const statusRes = await fetch(`${base}/api/eodhd/status`);
    const status = await statusRes.json();
    if (!status.configured) {
      console.log('EODHD_SMOKE_RESULT');
      console.log(JSON.stringify({ configured: false, blocker: 'EODHD_API_KEY_NOT_CONFIGURED' }, null, 2));
      return;
    }
    ...
    if (!last) throw new Error('Yahoo no devolvió barras para EUN6.DE');

    const crossRes = await fetch(`${base}/api/eodhd/cross-check`, {
      method: 'POST',
    ...
    console.log(JSON.stringify({
      ...
      summaryState: cross.summaryState,
      checked: cross.checked,
      matched: cross.matched,
      divergent: cross.divergent,
      upstreamCalls: cross.upstreamCalls,
      cacheHits: cross.cacheHits,
      status,
...
main().catch(err => {
  console.error('EODHD_SMOKE_ERROR', err?.message || String(err));
  process.exit(1);
});
